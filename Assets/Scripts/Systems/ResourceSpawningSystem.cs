...
public class ResourceSpawningSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        {
            for (int z = 0; z < spawner.gridZ; ++z)
            {
                for (int x = 0; x < spawner.gridX; ++x)
                {
                    var instance = ecb.Instantiate(spawner.ResourcePrefab);
                    var translation = spawnerTranslation.Value + new float3(x - (spawner.gridX - 1) / 2f, 0, z - (spawner.gridZ - 1) / 2f);
